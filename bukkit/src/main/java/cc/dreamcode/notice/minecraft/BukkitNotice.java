...
public class BukkitNotice extends NoticeImpl<BukkitNotice> implements BukkitSender {
  ...
  private void sendFormatted(@NonNull CommandSender target) {
    if (!(target instanceof Player player)) {
      ...
      Arrays.stream(split).forEach(text -> target.sendMessage(StringColorUtil.fixColor(text)));
      return;
    }
    ...
    switch (noticeType) {
      ...
      case CHAT:
        {
          String[] split = this.getRender().split(NoticeImpl.lineSeparator());
          Arrays.stream(split).forEach(text -> player.sendMessage(StringColorUtil.fixColor(text)));
          break;
        }
      case ACTION_BAR:
        {
