...
public class AdventureNotice<R extends Notice<R>> extends NoticeImpl<R> {
  ...
  public TextReplacementConfig getPlaceholderConfig() {
    ...
    Map<String, String> renderedFields =
        placeholderContext.renderFields().entrySet().stream()
            .collect(Collectors.toMap(entry -> entry.getKey().getRaw(), Map.Entry::getValue));
            ...
            (result, input) -> {
              String fieldValue =
                  ChatColor.translateAlternateColorCodes(
                      ALT_COLOR_CHAR, renderedFields.get(result.group(1)));
  ...
  public Component toJoiningComponent() {
    ...
    if (this.joiningComponent == null) {
      this.joiningComponent =
          AdventureLegacy.joiningDeserialize(this.getRaw(), this.getPlaceholderConfig());
    }
