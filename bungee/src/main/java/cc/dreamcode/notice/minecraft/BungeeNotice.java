...
public class BungeeNotice extends NoticeImpl<BungeeNotice> implements BungeeSender {
  ...
  private void sendFormatted(@NonNull CommandSender target) {
    if (!(target instanceof ProxiedPlayer player)) {
      ...
      Arrays.stream(split)
          .forEach(text -> target.sendMessage(new TextComponent(StringColorUtil.fixColor(text))));
      return;
    }
      ...
      case CHAT:
        {
          ...
          Arrays.stream(split)
              .forEach(
                  text -> player.sendMessage(new TextComponent(StringColorUtil.fixColor(text))));
          break;
        }
      case ACTION_BAR:
        {
          player.sendMessage(
              ...
              new TextComponent(
                  StringColorUtil.fixColor(
                      this.getRender().replace(NoticeImpl.lineSeparator(), ""))));
          break;
        }
      case TITLE:
        {
